...
const isLoggedIn = async (request: UserRequest, reply: FastifyReply) => {
  ...
  try { 
    const result = await client.query('SELECT user_id FROM session WHERE session_id = $1', [sessionId]) 
...
const logout = async (request: UserRequest, reply: FastifyReply) => {
  const sessionId = request.session.sessionId
  try { 
    await client.query('DELETE FROM session WHERE session_id = $1', [sessionId]) 
...
const login = async (request: UserRequest, reply: FastifyReply) => {
  ...
  try { 
    ...
    const deleteSuccess = await deletePreviousLogins(user.rows[0].id)
    console.log('DELETESUCCESS: ', deleteSuccess)

    await client.query('INSERT INTO session(session_id, user_id) VALUES($1, $2)', [request.session.sessionId, user.rows[0].id])
...
const deletePreviousLogins = async (id: number) => {
  try { 
...
const getProfile = async (request: UserRequest, reply: FastifyReply) => {
  try { 
    ...
    if(loggedInUser.success) {
      const user = await client.query('SELECT id, username FROM "user" WHERE id = $1', [loggedInUser.id])  
      return user.rows[0]
    }else {
      throw new Error()
    }
